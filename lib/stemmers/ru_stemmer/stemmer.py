...
def cleaning(text, punctuation_whitelist=['.','!','?'], stopword_removal=False):
    ...
    def replace(words, text, punctuation_distancing=False, with_spaces=True):
        for word in words:
            if with_spaces:
                word = ' {} '.format(word)
            if word in text:
                text = text.replace(word, ' ')
            if punctuation_distancing:
                text = text.replace(word, ' {} '.format(word))
        return text
    ...
    symbols.difference_update(set(punctuation_whitelist))
    text = replace(punctuation_whitelist, text, punctuation_distancing=True)
